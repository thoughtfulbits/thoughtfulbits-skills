...
description: "Pre-review a B2B SaaS board deck before it is sent to the board. Checks that the deck speaks plainly (no jargon or buzzwords), that the product and business strategy is stated as one meaningful sentence (the customer pain and how it is solved), that product plans cite real customer evidence (how many customers were talked to, what was learned), that the competitive picture is honest (who we are winning and losing against, with data), that every section pairs a clear 'what' with a real 'why', that the deck shows plan-vs-actual accountability and closes the loop on prior commitments, that cash and runway math is stated plainly, and that it ends with explicit asks. Use this skill whenever the user wants a board deck, board update, board pre-read, investor update, or quarterly/annual board materials reviewed, critiqued, pressure-tested, or checked before sending — even if they only say 'take a look at my board deck' or attach board materials as a .pptx, .pdf, .docx, .md, or pasted text."
---
